...
! first order continuation runs
   ...
   ! If Hopf-beta bifurcation calculations are being done, then branch
   ! here to do the rest of the algorithm.
   ELSE IF (con%general_info%method == HOPF_BETA_CONTINUATION) THEN

#if DEBUG > 2
      WRITE(*,*) '~~~~~~~~~~~~~~~~~~~~~~~~~  Passato 2.2.7  ~~ max delta_x =', MAXVAL(ABS(delta_x))
#endif

      converged = hopf_beta_alg(x, delta_x, con, reltol, abstol)

#if DEBUG > 2
      WRITE(*,*) '~~~~~~~~~~~~~~~~~~~~~~~~~  Passato 2.2.8  ~~ converged =', converged
      WRITE(*,*) '~~~~~~~~~~~~~~~~~~~~~~~~~  Passato 2.2.9  ~~ max delta_x =', MAXVAL(ABS(delta_x))
#endif
...
FUNCTION hopf_beta_alg(x, delta_x, con, reltol, abstol) RESULT (output)

! Algorithm for locking on to and tracking a Hopf point when Lns /= Jacobian
   IMPLICIT NONE
   ...
   IF (first == TRUE) THEN
      ...
      IF ((tmp == 0d0) .OR. (tmp2 == 0d0)) THEN
         ...
         IF (con%general_info%printproc > 1) &
            WRITE(*,*) 'ERROR in Hopf beta alg: complex vector pair must be supplied'
      ...
      ELSE IF (con%hopf_info%mass_flag == 5) THEN
         ...
         WRITE(*,*) 'Stop being lazy and know for yourself if the Mass matrix depends or not on the parameters'
         WRITE(*,*) 'STOP'
         STOP
